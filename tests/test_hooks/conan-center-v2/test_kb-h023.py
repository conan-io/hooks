...
class TestKBH023(ConanClientV2TestCase):
    ...
    def _get_environ(self, **kwargs):
        kwargs = super(TestKBH023, self)._get_environ(**kwargs)
    ...
    def test_regular_conandata_config(self):
        ...
        assert "pre_export(): [CONFIG.YML HAS NEW VERSION (KB-H023)] OK" in output
    ...
    def test_missing_version_in_config(self):
        ...
        save(self, os.path.join('all', 'conanfile.py'), content=self.conanfile)
        output = self.conan(['export', 'all', '--name=name', '--version=0.1.0'])
        assert 'pre_export(): ERROR: [CONFIG.YML HAS NEW VERSION (KB-H023)] The version "2.0" exists in "conandata.yml"' in output
